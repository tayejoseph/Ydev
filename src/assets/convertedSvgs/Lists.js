import * as React from "react";

function SvgLists(props) {
  return (
    <svg
      width="1em"
      height="1em"
      viewBox="0 0 58 66"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      xmlnsXlink="http://www.w3.org/1999/xlink"
      {...props}
    >
      <path fill="url(#Lists_svg__pattern0)" d="M0 0h58v66H0z" />
      <defs>
        <pattern
          id="Lists_svg__pattern0"
          patternContentUnits="objectBoundingBox"
          width={1}
          height={1}
        >
          <use
            xlinkHref="#Lists_svg__image0_2228:2062"
            transform="scale(.01724 .01515)"
          />
        </pattern>
        <image
          id="Lists_svg__image0_2228:2062"
          width={58}
          height={66}
          xlinkHref="data:image/png;base64,"
        />
      </defs>
    </svg>
  );
}

export default SvgLists;
